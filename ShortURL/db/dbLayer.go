...
/*
	this is the database layer used in the program.The GoLinkShorter API uses a mongo database as a backend.
	The database layer uses the mg library
	It follows a common connectoin pattern where a main session is created then other sessions are created by coping the information
	of the main session while utilizing a different socket from a socket pool
*/
...
const CONNECTIONSTRING = "mongodb://0.0.0.0:5099" //"http://localhost:5099/" //const CONNECTIONSTRING = "mongodb://127.0.0.1"
...
func (c *MongoConnection) createLocalConnection() (err error) {
	...
	if err == nil {
		...
		fmt.Printf("Error occured while creating mongodb connection: %s\n", err.Error())
	}
	return
}
